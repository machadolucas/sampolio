...
**PSU headers (attended calls).** A user-triggered flow carries a `PsuContext { ip?, userAgent? }`: the client sends `Psu-Ip-Address` and, when known, `Psu-User-Agent`, which tells the ASPSP a real person is behind the call and grants a higher rate allowance. The two travel **as a pair or not at all** — the client emits nothing unless `ip` is present, so a user agent is never sent alone. Both request paths build it with `psuContextFrom` (`src/lib/bank/psu-context.ts`): `getPsuContext` (`src/lib/actions/bank.ts`) from `headers()`, the consent callback from its request. The IP is `clientIpFrom` (`src/lib/rate-limit.ts`, the same order as Better Auth and the proxy limiter): `cf-connecting-ip` first, because Cloudflare *appends* to `X-Forwarded-For` and the first hop on the tunnel path is whatever the client sent; then the first `X-Forwarded-For` hop, which on the LAN path is Caddy's (it replaces the header). With neither header the IP is left out, never sent as "unknown", and the call counts as unattended. The scheduler passes no context, keeping unattended runs header-free — which is what makes "attended ⇔ `psuIp` present" a single switch for `run.psuPresent` and `shouldFetchPending` (section 6).
...
3. **Callback** — `GET src/app/api/bank/callback/route.ts` (one of the app's two non-auth API routes, alongside the avatar image endpoint):
   ...
   - Calls `completeConnection(userId, code, state, { psuIp, psuUserAgent })` with the `psuContextFrom` context (section 3, PSU headers).
