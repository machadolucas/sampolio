...
| 8 | **No passkey attestation.** Registration uses `attestation: "none"` (plugin default), so AAGUIDs are often all-zero (Apple) and the provider name falls back to "Passkey"; users rename them. Authenticator model is not verified. | `@better-auth/passkey`, `src/lib/db/passkeys.ts` |
