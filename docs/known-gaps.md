...
| 2 | **No end-to-end tests; component coverage is partial.** Pure engines, schemas, the db layer, a representative action-layer slice, the shared UI primitives, and the three highest-state components (cashflow item modal, split editor, reconcile wizard) are covered (~40 vitest files); most pages and server actions still have no automated coverage. The passkey flow (sign-up → password sign-in → add passkey → passkey/conditional-UI sign-in → rename/delete) was smoke-tested once with Playwright + a CDP virtual authenticator, but no browser suite is committed. Tracked as [`improvements.md`](improvements.md) §13.1. | `src/**/*.test.{ts,tsx}` |
...
| 6 | **Split storage: auth in SQLCipher, everything else in `.enc` files.** Users/sessions/accounts/passkeys live in `sampolio.db`; all financial data (and `app-settings.enc`, `preferences.enc`) is still file-based. The legacy `users-index.enc` / `users/*/user.enc` stay on disk after the one-shot import as the rollback path and are never updated, so they drift from the DB (names, roles, new users). The full-data migration follows the per-module pattern in [`src/lib/db/AGENTS.md`](../src/lib/db/AGENTS.md). | `src/lib/db/sqlite/`, `src/lib/db/users.ts` |
| 7 | **Brute-force lockout is in-memory and per process.** `isAccountLocked` / `recordFailedLogin` (`src/lib/db/users.ts`, also keyed `pw:<userId>` for `changeMyPassword`) and the sign-up action limits (`src/lib/rate-limit.ts`) reset on restart; Better Auth's own HTTP rate limits are DB-backed (`rateLimit` table). The `/passkey/*` routes are covered only by the rate limits, not the per-email lockout (they carry no email). | `src/lib/db/users.ts`, `src/lib/auth/server.ts` |
| 8 | **No passkey attestation.** Registration uses `attestation: "none"` (plugin default), so AAGUIDs are often all-zero (Apple); the default name then falls back to the registering browser ("Safari on iPhone"), which says where the passkey was created, not which password manager holds it; users can rename. Authenticator model is not verified. | `@better-auth/passkey`, `src/lib/passkey-name.ts` |
