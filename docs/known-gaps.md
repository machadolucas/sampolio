...
| 7 | **Brute-force lockout is in-memory and per process.** `isAccountLocked` / `recordFailedLogin` (`src/lib/db/users.ts`) reset on restart; Better Auth's own rate limits are DB-backed (`rateLimit` table). The `/passkey/*` routes are covered only by the rate limits, not the per-email lockout (they carry no email). | `src/lib/db/users.ts`, `src/lib/auth/server.ts` |
